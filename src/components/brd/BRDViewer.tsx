...
import { MessageSquare, History, FileText, Download, Edit3, Check, X, Lock, Send, RotateCcw, Clock, Code, HardDrive, File as FileIcon, ChevronDown, ChevronUp } from "lucide-react";
...
interface BRDViewerProps {
    brd: BRDRecord;
    onStatusChange: (id: string, status: BRDRecord["status"]) => void;
...
const STATUS_CONFIG: Record<string, { bg: string; text: string; border: string }> = {
    ...
    "Analyst BRD Review": { bg: "bg-purple-100", text: "text-purple-700", border: "border-purple-300" },
    "Under Development": { bg: "bg-purple-100", text: "text-purple-700", border: "border-purple-300" },
    "UAT Testing": { bg: "bg-orange-100", text: "text-orange-700", border: "border-orange-300" },
...
function getTimelineIndex(status: string) {
    ...
    if (status === "BRD Generated" || status === "Draft" || status === "Revision Required") return 0;
    if (status === "Approved") return 1;
    if (status === "Analyst BRD Review") return 2;
    if (status === "Under Development") return 3;
    if (status === "UAT Testing") return 4;
    if (status === "Production") return 5;
    return 0;
}

function EditableSection({ value, isLocked, onSave }: {
    value: string; isLocked: boolean; onSave: (v: string) => void;
}) {
        ...
        <div className="group relative">
            {editing ? (
                ...
                <div className="relative">
                    <pre className="whitespace-pre-wrap text-sm text-slate-600 leading-relaxed font-sans bg-slate-50/50 p-4 rounded-xl border border-slate-100">{value}</pre>
                    {!isLocked && (
                        <button
                            onClick={() => setEditing(true)}
                            className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-[#00338D] hover:border-[#00338D] transition-all interactive shadow-sm"
                        >
                            <Edit3 size={13} />
                        </button>
                    )}
...
export default function BRDViewer({ brd, onStatusChange, onAddComment, onSectionEdit, onRequestMajorChange }: BRDViewerProps) {
    ...
    const isReadOnly = brd.isLocked || ["Analyst BRD Review", "Approved", "Under Development", "UAT Testing", "Production", "Archived"].includes(brd.status);
    ...
    const handleStatus = (status: BRDRecord["status"]) => {
        setAnimKey(k => k + 1);
        onStatusChange(brd.id, status);
    };
    ...
    const inDev = ["Analyst BRD Review", "Under Development", "UAT Testing", "Production"].includes(brd.status);
    ...
    return (
        <div className="flex-1 min-w-0 flex flex-col">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {brd.status !== "Archived" && (
                            <div className="flex gap-2">
                                {!inDev ? (
                                    <>
                                        {brd.status !== "Revision Required" && (
                                            <button onClick={() => handleStatus("Revision Required")}
                                        ...
                                        {brd.status === "Approved" && (
                                            <button onClick={() => handleStatus("Analyst BRD Review")}
                                    ...
                                    onRequestMajorChange && (
                                        <div className="flex gap-2">
                                            {!isReadOnly && (brd.status === "BRD Generated" || brd.status === "Revision Required") && (
                                                <button onClick={() => handleStatus("Analyst BRD Review")}
                                                    className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold bg-[#00338D] text-white hover:bg-[#00266e] rounded-xl transition-all interactive shadow-sm">
                                                    <Code size={13} /> Send to Analyst
                                                </button>
                                            ...
                                            <button
                                                ...
                                                <RotateCcw size={13} /> Request Major Revision
                                            </button>
                                        </div>
                                    )
                                )}
            ...
            {/* Tracking Timeline */}
            <div className="bg-white border border-slate-200 rounded-xl p-5 mb-4 shadow-sm relative w-full">
            ...
            {/* Tabs */}
            <div className="flex border-b border-slate-200 bg-white rounded-t-xl mb-0 px-4">
                {([
            ...
            <div className="bg-white border border-t-0 border-slate-200 rounded-b-xl flex-1 overflow-y-auto">
                <AnimatePresence mode="wait">
                    {/* DOCUMENT TAB */}
                    {tab === "document" && (
                        <motion.div key="doc" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-6 space-y-8">
